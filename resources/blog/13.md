...
**EDIT 6/13/2024:** Turns out someone *has* done this before, and not just done it, but done it prolifically and well. David Cohen's been making ["Scrabblegrams,"](https://davesscrabblegrams.com/) so-called, since 1997. Guy even published a book. He might be way better at at writing them than me, but I'll be sticking to my name for them, thank you very much.
...
badgeDescriptions[Badge.Dome] = "No anagram tools or thesauri — this one was all you. And maybe the dictionary.";
	...
	try {
		Promise.all(promises).then((values) => Promise.all(values.map((v, i) => {
			...
			for (const r of poems) {
				...
				appendPoem(r.tom === true, poem, author, r.timestamp, r.dome);
			}
...
const appendPoem = (isTom, poem, author, timestamp, dome) => {
