...
Those are pretty good, even if they use some letters multiple times. If you don't allow that, you get something like:
...
Those blank tiles are pretty clutch. You should give it a try! Here are some handy tools:
...
const url = 'https://corsproxy.io/?' + encodeURIComponent(listURL);
const fetchScroems = () => {
...
const submitPoem = async (author, poem) => {
	const url = 'https://corsproxy.io/?' + encodeURIComponent(postURL.replace('{0}', poem).replaceAll('{1}', author));
	window.fetch(url, {
		method: 'POST',
	}).then(fetchScroems);
}
